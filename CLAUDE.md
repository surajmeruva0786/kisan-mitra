...
- **2026-08-02**: End-to-end smoke test — ran `npm test` in `backend/` (9/9 passing), then started both dev servers together and hit `/api/health`, `/api/schemes`, `/api/weather`, and `POST /api/chat` **through the frontend's own port** (`localhost:5173/api/...`) to prove the Vite dev proxy genuinely reaches the backend, not just backend-in-isolation. Also ran `npm run build` + `npm run preview` to confirm the production build serves correctly. This closes out the full-stack rebuild: backend (7 steps) + frontend (13 steps) + docs, all committed and pushed incrementally.
